...
public class VodInfo implements Serializable {
    ...
    public void setVideo(Movie.Video video) {
        ...
        des = video.des;
        if (video.urlsBean != null && video.urlsBean.flagInfoList != null && video.urlsBean.flagInfoList.size() > 0) {
            ...
            for (Movie.Video.UrlsBean.FlagUrlsInfo urlInfo : video.urlsBean.flagInfoList) {
                if (urlInfo.nameUrlBeans != null && urlInfo.nameUrlBeans.size() > 0) {
                    List<VodSeries> seriesList = new ArrayList<>();
                    for (Movie.Video.UrlsBean.FlagUrlsInfo.NameUrlBean infoBean : urlInfo.nameUrlBeans) {
            ...
            SiteBean sb = ApiConfig.getInstance().getSiteBean(video.sourceKey);
            if (sb != null) { // ssp 不排序
                // 优先展示m3u8
