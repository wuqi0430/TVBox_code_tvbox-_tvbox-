...
public class AbsJson implements Serializable {
    ...
    public AbsXml toAbsXml() {
        ...
        movie.pagesize = StringUtil.string2Integer0(limit);
    ...
    public class AbsJsonVod implements Serializable {
        ...
        public Movie.Video toXmlVideo() {
            ...
            video.tid = type_id;
            video.name = vod_name;
            video.type = NullUtil.isNull(type_name) ? vod_class : type_name;
            // video.dt = vod_play_from == null ? "" : vod_play_from.replace("$$$", ",");
            video.pic = vod_pic;
            video.lang = vod_lang;
            video.area = vod_area;
            video.year = StringUtil.string2Integer0(vod_year);
            video.state = vod_state;
            video.note = NullUtil.isNull(vod_remarks) ? vod_duration : vod_remarks;
            video.actor = vod_actor;
            video.director = vod_director;
            Movie.Video.UrlsBean urlBean = new Movie.Video.UrlsBean();
            if (vod_play_from != null && vod_play_url != null) {
                String[] playFlags = vod_play_from.split("\\$\\$\\$");
                String[] playUrls = vod_play_url.split("\\$\\$\\$");
                List<Movie.Video.UrlsBean.FlagUrlsInfo> flagInfoList = new ArrayList<>();
                for (int i = 0; i < playFlags.length; i++) {
                    Movie.Video.UrlsBean.FlagUrlsInfo urlInfo = new Movie.Video.UrlsBean.FlagUrlsInfo();
                    ...
                    flagInfoList.add(urlInfo);
                }
                urlBean.flagInfoList = flagInfoList;
            }
            video.urlsBean = urlBean;
